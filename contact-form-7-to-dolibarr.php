...
class Wpcf7_to_Dolibarr_Plugin
{
	...
	public function display_panel($form) {
		if (wpcf7_admin_has_edit_cap()) {
			$options = get_option('wpcf7_dolibarr_'.$form->id);
	...
	public function sync_into_dolibarr($form) {
        ...
        $existingContact = $dolibarrSync->searchContact($options['field_email']);

        $searchCompany = $existingContact ?: $dolibarrSync->searchCompany($options['field_email']);
